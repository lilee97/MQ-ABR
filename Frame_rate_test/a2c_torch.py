...
class ActorNet(nn.Module):
    def __init__(self, s_dim, a_dim, lr):
        ...
        self.out_linear = nn.Linear(3200, 9)
    ...
    def forward(self, inputs):
        ...
        split_6 = F.relu(self.fc4(inputs[:, 6:7, -1]))
        split_7 = F.relu(self.fc4(inputs[:, 7:8, -1]))
        split_8 = F.relu(self.fc4(inputs[:, 8:9, -1]))
        split_9 = F.relu(self.fc4(inputs[:, 9:10, -1]))
        # split_10 = F.relu(self.fc4(inputs[:, 10:11, -1]))
        # split_11 = F.relu(self.fc4(inputs[:, 11:12, -1]))
        ...
        split_2_flatten, split_3_flatten, split_4_flatten = split_2.flatten(start_dim=1), split_3.flatten(
                start_dim=1), split_4.flatten(start_dim=1)
        # print(split_2_flatten.size(), split_3_flatten.size())
        merge_net = torch.cat(
            [split_0, split_1, split_2_flatten, split_3_flatten, split_4_flatten, split_5, split_6, split_7, split_8, split_9], dim=1)
...
class CriticNet(nn.Module):
    def __init__(self, s_dim, lr):
        ...
        self.fc3 = nn.Linear(1, 128)
        self.fc4 = nn.Linear(1, 128)
        self.out_linear = nn.Linear(3200, 1)
    ...
    def forward(self, inputs):
        ...
        merge_net = torch.cat([split_0, split_1, split_2_flatten, split_3_flatten, split_4_flatten, split_5, split_6, split_7, split_8, split_9],
                              dim=1)
    ...
    def cal_loss(self, s_batch, r_batch, terminal):
        ...
        v_out, v_batch = self.get_value_out(s_batch)  # 将动作输入网络，得到俩结果
